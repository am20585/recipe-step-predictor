...
| name                                 |   minutes | tags                                                                                                                                                                                                                                                                                               |   n_steps |   n_ingredients |   calories |   total_fat_PDV |   protein_PDV |
|:-------------------------------------|----------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------:|----------------:|-----------:|----------------:|--------------:|
| 1 brownies in the world    best ever |        40 | ['60-minutes-or-less', 'time-to-make', 'course', 'main-ingredient', 'preparation', 'for-large-groups', 'desserts', 'lunch', 'snacks', 'cookies-and-brownies', 'chocolate', 'bar-cookies', 'brownies', 'number-of-servings']                                                                        |        10 |               9 |      138.4 |              10 |             3 |
| 1 in canada chocolate chip cookies   |        45 | ['60-minutes-or-less', 'time-to-make', 'cuisine', 'preparation', 'north-american', 'for-large-groups', 'canadian', 'british-columbian', 'number-of-servings']                                                                                                                                      |        12 |              11 |      595.1 |              46 |            13 |
| 412 broccoli casserole               |        40 | ['60-minutes-or-less', 'time-to-make', 'course', 'main-ingredient', 'preparation', 'side-dishes', 'vegetables', 'easy', 'beginner-cook', 'broccoli']                                                                                                                                               |         6 |               9 |      194.8 |              20 |            22 |
| millionaire pound cake               |       120 | ['time-to-make', 'course', 'cuisine', 'preparation', 'occasion', 'north-american', 'desserts', 'american', 'southern-united-states', 'dinner-party', 'holiday-event', 'cakes', 'dietary', 'christmas', 'thanksgiving', 'low-sodium', 'low-in-something', 'taste-mood', 'sweet', '4-hours-or-less'] |         7 |               7 |      878.3 |              63 |            20 |
| 2000 meatloaf                        |        90 | ['time-to-make', 'course', 'main-ingredient', 'preparation', 'main-dish', 'potatoes', 'vegetables', '4-hours-or-less', 'meatloaf', 'simply-potatoes2']                                                                                                                                             |        17 |              13 |      267   |              30 |            29 |
...
## Baseline Model
Our baseline model is a **linear regression** model aimed at predicting the number of steps (`n_steps`). We used two quantitative features: `total_fat_PDV` (quantitative) and `protein_PDV` (quantitative). These features were chosen based on the assumption that recipes with higher fat and protein content might be more complex and require a greater number of steps.
...
## Final Model
In our final model, we added two more features: `n_ingredients` (quantitative) and `calories` (quantitative). We believe these two features have a high impact on the number of steps as when increasing the number of ingredients of a dish, we expect utilizing all those ingredients would typically increase the number of steps. We also added calories for a similar reason as using total fat and protein.

The modeling process involved testing multiple algorithms, including **Ridge Regression**, **Lasso Regression**, **ElasticNet**, and **Gradient Boosting Regressor**. These models were selected to balance flexibility and regularization. The hyperparameters for each model were tuned using GridSearchCV with a robust cross-validation strategy (RepeatedKFold) to make sure that the model performed well on unseen data. For ElasticNet, the best hyperparameters were an alpha value of 0.1 for regularization strength and an l1_ratio of 0.5, balancing L1 and L2 penalties. For the Gradient Boosting Regressor, the best hyperparameters included a learning_rate of 0.1, 200 n_estimators, and a max_depth of 5, which allowed the model to capture some of the complex, nonlinear relationships in this dataset. 

Our new Mean Absolute Error is down to **3.984** showing our estimations are only about 4 steps off on average. Our R² value is now **0.2192**, meaning our model has improved quite a lot in this aspect as well. We have achieved some improvement in both of our evaluation metrics, suggesting that we made an effective adjustment in our final model. These metrics show that the final model provides a more accurate and reliable prediction of n_steps. We believe that this improvement occured because the number of ingredients in a recipe inherently relates to its complexity and the calories per serving reflect the overall energy density of a dish, which often corresponds to the richness and variety of ingredients, thus this feature provides an indirect measure of recipe complexity that complements `n_ingredients`.

We also experimented with one-hot encoding the `tags` column, but it did not significantly improve the final model’s performance, which is likely due to the high complexity and sparsity of the tags. This may not have effectively captured meaningful patterns related to `n_steps`.

To conclude and to answer our question of what factors influence the number of steps in a recipe, our analysis shows that the number of ingredients and nutritional factors like total_fat_PDV, protein_PDV, and calories play a very important role in predicting recipe complexity. Recipes with higher ingredient counts and greater nutritional values tend to involve more steps, though the relationship is not strictly linear.

While our final model showed improved performance over the baseline, the relatively modest R² tells us that there are still unexplored factors influencing recipe complexity. Exploring tags and additional data columns might give us further insights into this, and more advanced models could potentially catch the nuances in this dataset. Nevertheless, our findings show the importance of nutritional and ingredient-related features in understanding recipe complexity.
